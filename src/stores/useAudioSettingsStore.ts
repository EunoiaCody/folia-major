...
/** VIP 歌曲解锁总开关（网易云 VIP/试听曲目 unblock 完整音源）。 */
export const UNLOCK_VIP_SONGS_KEY = 'unlock_vip_songs';
/** 无版权歌曲自动替代总开关（灰色歌曲按需用 unblock / 跨 provider 替代播放）。 */
...
export type AudioSettingsState = {
    ...
    /** VIP 歌曲解锁：网易云标准授权失败时先用同平台 unblock 完整音源，再按需跨 provider 替换。 */
    unlockVipSongs: boolean;
    /** 无版权歌曲自动替代：灰色歌曲不再被网格/队列过滤，播放时尝试 unblock 或跨 provider 匹配。 */
    unlockUnavailableSongs: boolean;
    /** 同平台 unblock 仍不可用时，是否允许跨 provider（酷狗/QQ）搜索同曲音源。 */
...
export const useAudioSettingsStore = create<AudioSettingsState>((set, get) => ({
    ...
    unlockVipSongs: getStoredBoolean(UNLOCK_VIP_SONGS_KEY, true),
    unlockUnavailableSongs: getStoredBoolean(UNLOCK_UNAVAILABLE_SONGS_KEY, true),
    unlockUseCrossProviderFallback: getStoredBoolean(UNLOCK_CROSS_PROVIDER_FALLBACK_KEY, true),
    ...
    handleToggleUnlockVipSongs: (enable) => {
        setStoredBoolean(UNLOCK_VIP_SONGS_KEY, enable);
    ...
    handleToggleUnlockUnavailableSongs: (enable) => {
        setStoredBoolean(UNLOCK_UNAVAILABLE_SONGS_KEY, enable);
    ...
    handleToggleUnlockUseCrossProviderFallback: (enable) => {
        setStoredBoolean(UNLOCK_CROSS_PROVIDER_FALLBACK_KEY, enable);
...
export const selectAudioSettingsSnapshot = (state: AudioSettingsState) => ({
    ...
    handleToggleUnlockVipSongs: state.handleToggleUnlockVipSongs,
    handleToggleUnlockUnavailableSongs: state.handleToggleUnlockUnavailableSongs,
    handleToggleUnlockUseCrossProviderFallback: state.handleToggleUnlockUseCrossProviderFallback,
});
...
export const handleToggleUnlockVipSongs = (enable: boolean) => useAudioSettingsStore.getState().handleToggleUnlockVipSongs(enable);
export const handleToggleUnlockUnavailableSongs = (enable: boolean) => useAudioSettingsStore.getState().handleToggleUnlockUnavailableSongs(enable);
export const handleToggleUnlockUseCrossProviderFallback = (enable: boolean) => useAudioSettingsStore.getState().handleToggleUnlockUseCrossProviderFallback(enable);
