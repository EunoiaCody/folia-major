...
- `unlockService.ts`（app 级，非 provider）：VIP 解锁编排，显式跨 provider。`resolveUnlockedAudioSource` 先标准授权，再同平台 unblock，最后按酷狗→QQ 顺序用 `calculateMatchScoreDetails`（复用 `src/utils/lyrics/matchScore.ts`）语义匹配同曲并取音源；匹配要求标题命中 + 歌手或专辑命中 + 分数 ≥ 75。受 `useSettingsUiStore` 的 `unlockVipSongs` / `unlockUseCrossProviderFallback` 控制，关闭时保持原播放行为。
